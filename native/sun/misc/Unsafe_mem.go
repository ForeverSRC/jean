...
func _unsafe(method func(frame *jvmstack.Frame), name, desc string) {
	native.Register(constants.SunMiscUnsafe, name, desc, method)
}

// public native long allocateMemory(long bytes);
// (J)J
func allocateMemory(frame *jvmstack.Frame) {
...
// (JJ)J
func reallocateMemory(frame *jvmstack.Frame) {
	vars := frame.LocalVars()
	// vars.GetRef(0) // this
...
// (J)V
func freeMemory(frame *jvmstack.Frame) {
	vars := frame.LocalVars()
	// vars.GetRef(0) // this
	address := vars.GetLong(1)
	free(address)
...
// (J)B
func mem_getByte(frame *jvmstack.Frame) {
...
// (JJ)V
func mem_putLong(frame *jvmstack.Frame) {
	vars := frame.LocalVars()
	// vars.GetRef(0) // this
	address := vars.GetLong(1)
...
//}
//
func _get(frame *jvmstack.Frame) (*jvmstack.OperandStack, []byte) {
	vars := frame.LocalVars()
	// vars.GetRef(0) // this
	address := vars.GetLong(1)
