...
func getDeclaredFields0(frame *jvmstack.Frame) {
	...
	if fieldCount > 0 {
		...
		for i, goField := range fields {
			...
			ops := jvmstack.NewOperandStack(8)
...
func init() {
	native.Register(constants.JavaLangClass, "getDeclaredFields0", "(Z)[Ljava/lang/reflect/Field;", getDeclaredFields0)
}
